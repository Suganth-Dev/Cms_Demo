// City Hope Hospital Portal Script
let specialtiesData = [];
let isPreview = false;
...
if (window.self !== window.top) {
    isPreview = true;
    console.log("City Hope Hospital loaded in Preview Mode (Iframe)");
}
...
async function initHospitalPortal() {
    try {
        await checkTableExistence();
        
        // 1. Fetch & Apply settings
        const savedSettings = await getSettings('hospital');
        const mockDefaults = (typeof MULTI_SITE_DATA !== 'undefined' && MULTI_SITE_DATA.hospital)
            ? MULTI_SITE_DATA.hospital.settings || {}
            : {};
        const settings = savedSettings ? { ...mockDefaults, ...savedSettings } : { ...mockDefaults };
        applySettings(settings);

        // 2. Fetch & Render Specialties
        await loadSpecialties();

        // 3. Fetch & Render Intake Forms PDF downloads
        await loadDocuments();

        // 4. Fetch & Render Innovations marketing resources
        await loadMarketing();

        // 5. Fetch & Render Healthcare Affiliates
        await loadPartners();
        ...
        console.error("Hospital Portal initialization error:", err);
    }
}
...
// Fetch and render clinical specialties
async function loadSpecialties() {
    const grid = document.getElementById('specialties-grid-container');
    try {
        let list = await getProducts('hospital');
        ...
        specialtiesData = list;

        if (list.length === 0) {
            grid.innerHTML = `<p class="loading-placeholder">No clinical divisions loaded at this time.</p>`;
            return;
        }

        renderSpecialtiesGrid(list);
        setupFilterClicks();
...
function renderSpecialtiesGrid(list) {
    const grid = document.getElementById('specialties-grid-container');
    
    grid.innerHTML = list.map(p => {
        let badge = '';
        if (isPreview) {
            badge = `<span class="specialty-category-badge" style="right:16px; left:auto; background:var(--bg-dark); color:white; font-size:9px;">${p.status}</span>`;
        }

        return `
            <div class="specialty-card" data-category="${p.category}">
                <div class="specialty-card-preview">
                    <img src="${p.image_url || 'https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?w=500&auto=format&fit=crop&q=80'}" alt="Clinical Division Image">
                    <span class="specialty-category-badge">${p.category}</span>
                    ${badge}
                </div>
                <div class="specialty-card-content">
                    <h3 class="specialty-card-title">${p.title}</h3>
                    <p class="specialty-card-desc">${p.description || ''}</p>
                    <div class="specialty-card-action" onclick="openProductDetailModal('${p.id}')">
                        <span>Read Specialties Overview</span>
...
// Modal popup specialty detail
window.openProductDetailModal = function(id) {
    const p = specialtiesData.find(x => x.id === id);
    if (!p) return;
    ...
    if (brochureUrl) {
        brochureBlock.innerHTML = `
            <a href="${brochureUrl}" target="_blank" class="btn btn-primary" style="display:inline-flex; width:100%; align-items:center; justify-content:center; gap:8px; padding:10px 20px; text-decoration:none; background-color:var(--accent-color); color:white; font-weight:700; border-radius:var(--radius-sm);">
                <i data-lucide="download" style="width:16px; height:16px;"></i> Download Info Brochure
            </a>
        `;
...
// Fetch and render Intake forms technical pdfs
async function loadDocuments() {
    const tbody = document.getElementById('downloads-table-body');
    try {
        let list = await getDocuments('hospital');
        ...
        if (list.length === 0) {
            tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; color:var(--text-muted);">No files registered for patient intake.</td></tr>`;
            return;
        }
...
// Fetch and render innovations marketing resources
async function loadMarketing() {
    const container = document.getElementById('marketing-container');
    try {
        const list = await getMarketing('hospital');
        if (list.length === 0) {
        ...
        container.innerHTML = list.map(m => `
            <div class="marketing-card">
                <div class="video-container">
                    <video controls muted preload="none" poster="https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?w=800&auto=format&fit=crop&q=60">
...
async function loadPartners() {
    ...
    try {
        const list = await getPartners('hospital');
        if (list.length === 0) {
            container.innerHTML = `<p style="grid-column: span 4; text-align:center; color:var(--text-muted);">No affiliations listed.</p>`;
            return;
        }
        ...
        container.innerHTML = list.map(p => `
            <div class="partner-card">
                ...
                <h3>${p.name}</h3>
                ${p.website_link ? `<a href="${p.website_link}" target="_blank" class="partner-link">Visit Affiliate Website <i data-lucide="external-link"></i></a>` : ''}
            </div>
        `).join('');
        lucide.createIcons();
    } catch (e) {
...
function setupPreviewListener() {
    window.addEventListener('message', (event) => {
        if (event.data && event.data.type === 'cms-preview-update') {
            console.log("Hospital received customizer postMessage update");
